...
class ValensAmmoOverhaul implements IPostDBLoadMod {
    ...
    private processCaliber(caliber: string): void {
        ...
        and PenetrationPower mulitpliers per the configured values for each caliber.*/
        for (const ammoType in ammo) {
            const configAmmo = this.modConfig[caliber].find(x => x.BulletIDs.includes(ammoType));
            if (configAmmo.Enabled) {
                ammo[ammoType]._props.ArmorDamage *= configAmmo.ArmorDamage;
                ammo[ammoType]._props.Damage *= configAmmo.Damage;
                ammo[ammoType]._props.PenetrationPower *= configAmmo.PenetrationPower;
            }
        }
    }
}
